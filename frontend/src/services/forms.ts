 ...
 * Returns {status, feedback, message?, existing_feedback?}
 * - status='success': feedback was generated and saved
 * - status='feedback_exists': feedback already exists, need user confirmation to overwrite
 */
export async function generateAIFeedback(formId: string, overwrite: boolean = false): Promise<any> {
  const response = await apiClient.post(`/api/forms/${formId}/ai-feedback`, { overwrite });
  return response.data.data; // Backend returns {success, data, error}
}
